...
create table if not exists public.session_readiness_assessments (
  ...
  session_purpose text not null
    check (
      session_purpose in (
        'diagnostic', 'check_in', 'emergency', 'free_practice',
        'drill', 'text_exercise', 'priming', 'field_log_back'
      )
    ),
  ...
  overall_band text
    check (
      ...
      or overall_band in (
        'building_baseline', 'one_focus_area', 'early_reps',
        'solid_ground', 'sustained_in_practice'
      )
    ),
...
create table if not exists public.session_readiness_signals (
  ...
  null_reason text
    check (
      null_reason is null
      or null_reason in (
        'insufficient_member_turns', 'no_defined_counterpart',
        'no_friction_event', 'no_scenario_shift',
        'capture_quality_insufficient', 'signal_not_applicable'
      )
    ),
...
create table if not exists public.session_readiness_evidence (
  ...
  evidence_type text not null
    check (evidence_type in ('support', 'friction', 'recovery', 'breakdown')),
  turn_id text,
...
create index if not exists session_readiness_assessments_comparison_idx
  on public.session_readiness_assessments (
    user_id, scenario_family, session_purpose, modality, pressure_level,
    created_at desc
  );
...
create table if not exists public.forge_agent_runs (
  ...
  created_at timestamptz not null default now(),
  run_day date,
  constraint forge_agent_runs_draft_attempt_identity
    check (
      kind <> 'draft_attempt'
...
create unique index if not exists forge_agent_runs_draft_attempt_user_day_uidx
