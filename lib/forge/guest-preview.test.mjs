...
import {
  GUEST_FORGE_DEFAULT_DURATION_SECONDS,
...
describe("Decision 060 guest Forge preview", () => {
  it("maps every independent Coach topic to deterministic opening context", () => {
    for (const topic of COACH_TOPICS) {
      ...
      assert.match(context.objective, /one focused rep/);
      assert.match(context.objective, /close warmly/);
  ...
  it("defaults the guest duration to 5 minutes and clamps to at most 15", () => {
    assert.equal(
    ...
    assert.equal(clampGuestPreviewDurationSeconds(Number.NaN), 300);
    assert.equal(clampGuestPreviewDurationSeconds(1), 60);
    assert.equal(formatGuestPreviewCountdown(300), "5:00");
    assert.equal(formatGuestPreviewCountdown(61), "1:01");
    assert.equal(formatGuestPreviewCountdown(0), "0:00");
  });
      ...
      () =>
        assertSameOrigin(
          new Request("https://talkforge.test/api/forge/preview", {
            method: "POST",
            headers: { origin: "https://evil.test", host: "talkforge.test" },
          })
        ),
  ...
  it("keeps the authenticated Realtime/readiness branch and isolates guest writes", () => {
    ...
    assert.match(arena, /startGuestDurationWatch\(/);
    assert.match(arena, /guestStopStartedRef/);
    assert.match(arena, /function guestStartWasAborted\(/);
    assert.match(arena, /formatGuestPreviewCountdown\(guestSecondsRemaining\)/);
    assert.match(
      arena,
      /isGuestPreview && inSession \? \([\s\S]*onClick=\{\(\) => void handleStop\(\)\}/
    );
    assert.match(realtimeRoute, /voiceMode: "hold"/);
    assert.match(realtimeRoute, /handsFree: false/);
    const realtimeClient = readFileSync(
      new URL("../../lib/ce/realtime.ts", import.meta.url),
      "utf8"
    );
    assert.match(
      realtimeClient,
...
function assertReconnectDenied(request) {
  assert.throws(
    () => assertSameOrigin(request),
    (error) => error.status === 403 && error.code === "reconnect_denied"
  );
}

describe("assertSameOrigin request authority", () => {
  it("accepts a Next dev proxy Host that differs from request.url", () => {
    assert.doesNotThrow(() =>
      assertSameOrigin(
        new Request("http://localhost:3000/api/forge/preview", {
          method: "POST",
          headers: {
  ...
  it("accepts Host without forwarded headers when Origin matches", () => {
    assert.doesNotThrow(() =>
      assertSameOrigin(
        new Request("http://localhost:3000/api/forge/preview", {
          method: "POST",
  ...
  it("does not treat a present Origin as sufficient without Host", () => {
    assertReconnectDenied(
      new Request("https://talkforge.test/api/forge/preview", {
        method: "POST",
        headers: { origin: "https://talkforge.test" },
      })
    );
  });

  it("does not trust request.url when Origin matches it but Host differs", () => {
    assertReconnectDenied(
      new Request("http://localhost:3000/api/forge/preview", {
        method: "POST",
        headers: {
  ...
  it("rejects a hostile Origin against the Host authority", () => {
    assertReconnectDenied(
      new Request("https://talkforge.test/api/forge/preview", {
        method: "POST",
        headers: {
          ...
          host: "talkforge.test",
          "x-forwarded-host": "talkforge.test",
  ...
  it("rejects a spoofed forwarded host that disagrees with Host", () => {
    assertReconnectDenied(
      new Request("https://talkforge.test/api/forge/preview", {
        method: "POST",
        headers: {
  ...
  it("rejects multiple forwarded host values", () => {
    assertReconnectDenied(
      new Request("https://talkforge.test/api/forge/preview", {
        method: "POST",
        headers: {
          origin: "https://talkforge.test",
          host: "talkforge.test",
          "x-forwarded-host": "talkforge.test,evil.test",
          "x-forwarded-proto": "https",
        },
      })
    );
  ...
  it("rejects multiple forwarded proto values", () => {
    assertReconnectDenied(
      new Request("https://talkforge.test/api/forge/preview", {
        method: "POST",
        headers: {
  ...
  it("rejects a missing Origin even when Host is present", () => {
    assertReconnectDenied(
      new Request("https://talkforge.test/api/forge/preview", {
        method: "POST",
        headers: { host: "talkforge.test" },
      })
    );
  });

  it("uses a single forwarded proto when request.url scheme differs", () => {
    assert.doesNotThrow(() =>
      assertSameOrigin(
        new Request("http://localhost:3000/api/forge/preview", {
          method: "POST",
