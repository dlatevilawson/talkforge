...
describe("echo reference barge-in", () => {
  ...
  it("allows barge-in only when mic clearly talks over remote playback", () => {
    ...
    const micHistory = [0.2, 0.25, 0.48, 0.62, 0.72, 0.68, 0.7, 0.69];
    assert.equal(
      isConfirmedTalkOverBargeIn({
        micLevel: 0.69,
        remoteLevel: 0.22,
  ...
  it("does not let moderate background speech cancel Forge", () => {
    assert.equal(
      isConfirmedTalkOverBargeIn({
        micLevel: 0.5,
        remoteLevel: 0.18,
        ...
        modulation: 0.09,
        speechBandRatio: 0.5,
        micHistory: [0.12, 0.2, 0.35, 0.48, 0.5, 0.46, 0.5, 0.49],
        remoteHistory: [0.2, 0.18, 0.22, 0.17, 0.21, 0.19, 0.18, 0.2],
      }),
      false
    );
  });
