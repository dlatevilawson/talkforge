...
describe("voice economics", () => {
  it("keeps normal turns at or under the turn cap", () => {
    assert.ok(outputBudgetForTurn("normal", false) <= 120);
    assert.ok(outputBudgetForTurn("quick_followup", false) < outputBudgetForTurn("normal", false));
    assert.ok(outputBudgetForTurn("complex", false) > outputBudgetForTurn("normal", false));
  });
