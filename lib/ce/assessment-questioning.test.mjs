...
test("A. Forge introduces itself before diagnostic questioning", () => {
  ...
  assert.match(opening, /Opening OBJECTIVES|phrase in your own words/i);
  assert.match(opening, /Exactly one question mark/i);
  assert.doesNotMatch(opening, /nearly verbatim/i);
  ...
  assert.match(system, /OBJECTIVES, not a sticky script/i);
  assert.doesNotMatch(system, /nearly verbatim/i);
});
...
test("H. Context may wait — unresolved mechanism discrimination overrides slot filling", () => {
  ...
  // stay on the active gateway default for that slot but forbid context jump.
  ...
  assert.match(stuckOnContextSlot, /concrete_recall|recognition|forced_comparison/i);
  assert.doesNotMatch(stuckOnContextSlot, /Preferred next wording/i);
  assert.doesNotMatch(
    stuckOnContextSlot,
    /Who are you usually talking to when this gets hard/i
  );
});
...
test("I. Explicit stop intent remains respected", () => {
  ...
  // Soft seed for true process confusion still exists; not forced every turn.
  ...
  assert.match(turnOk, /process confusion about purpose/i);
  assert.doesNotMatch(turnOk, /nearly verbatim/i);
  assert.doesNotMatch(turnOk, new RegExp(ASSESSMENT_DISENGAGEMENT_CHECK_IN.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
});
...
test("coach brain is restored — assessment does not strip Forge philosophy", () => {
  const system = buildAssessmentSystemInstructions();
  assert.match(system, /Understand before you coach/i);
  assert.match(system, /more understood than evaluated/i);
  assert.match(system, /LISTEN → ANALYZE → RESPOND/i);
  assert.match(system, /MODE CAPABILITIES \/ APP OWNERSHIP|APPLICATION OWNS SLOT SELECTION/i);
  assert.match(system, /YOU own/i);
  assert.match(system, /THE APP owns/i);
  assert.match(system, /observe.*persist|persists what it learns|APP owns/i);
  assert.match(system, /high-stakes real-world conversations/i);
  assert.match(system, /FORGE CORE \(always-on/);
  assert.match(system, /never a questionnaire/i);
  ...
  assert.match(turn, /You are Coach Forge/i);
  assert.match(turn, /You own the next conversational move|You own the next question/i);
  assert.match(turn, /APP OBSERVATION TARGET/i);
  assert.match(turn, /do not fill a form/i);
  assert.match(turn, /Lead with coach judgment/i);
  assert.doesNotMatch(turn, /Preferred next wording/i);
  assert.doesNotMatch(turn, /Coach-preferred discriminating wording/i);
});
