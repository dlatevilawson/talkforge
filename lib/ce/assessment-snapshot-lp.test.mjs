...
describe("assessment snapshot → LP mapper (Step 7)", () => {
  ...
  it("sufficient snapshot maps diagnosis into LP (not raw slot echo)", () => {
    ...
    assert.deepEqual(mapped.goals, [
      snap.diagnosis.focusArea || snap.diagnosis.primaryBottleneck,
    ]);
    assert.equal(mapped.purposeStatement, snap.diagnosis.desiredOutcome);
    assert.ok(
      mapped.challenges.includes(
        snap.diagnosis.keyEnvironments || snap.diagnosis.contexts
      )
    );
    assert.ok(
      mapped.challenges.includes(
        snap.diagnosis.rootPattern || snap.diagnosis.supportingPatterns[0]
      )
    );
    ...
    assert.doesNotMatch(mapped.goals[0], /^small talk$/i);
  });
