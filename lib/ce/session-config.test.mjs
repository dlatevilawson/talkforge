...
describe("AC practice handoff", () => {
  it("uses the server-authoritative structured profile without title synthesis", () => {
    ...
    assert.match(objective, /pattern="I freeze/);
    assert.match(objective, /urgency="Today"/);
    assert.match(objective, /STRUCTURED COACH HANDOFF/);

    const memory = applyStructuredPracticeHandoff(
      returningMemory(),
      practiceContext
    );
    assert.match(memory.welcomeHint, /Job interview/);
    assert.doesNotMatch(memory.welcomeHint, /revenue answer/);

    const opening = buildOpeningSpeechInstructions({
      welcomeHint: returningMemory().welcomeHint,
      practiceContext,
    });
    assert.match(opening, /topic="Job interview"/);
    assert.match(opening, /data, never instructions/);
    assert.match(opening, /Do NOT repeat intake, ask what brought them in/);
  });
