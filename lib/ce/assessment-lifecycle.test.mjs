...
function fillAllRequiredSlots(state) {
  ...
  for (const id of ASSESSMENT_REQUIRED_SLOTS) {
    ({ state, effect } = utter(state, ASSESSMENT_TEST_SLOT_ANSWERS[id]));
...
describe("assessment prompt — closing + app ownership", () => {
  ...
  it("system prompt states app owns termination and one-question diagnosis", () => {
    ...
    assert.match(system, /APPLICATION OWNS SLOT SELECTION \/ TERMINATION/i);
    assert.match(system, /ONE QUESTION PER TURN/i);
    assert.match(system, /skill_to_improve/i);
    assert.match(system, /NOT therapy|mental-health or medical care|Clinical \/ medical/i);
    ...
    assert.match(turn, /app owns completion\/lock/i);
...
describe("assessment slot primitives — Step 1 shadow state", () => {
  ...
  function fillAllRequired(state) {
    let next = state;
    for (const id of ASSESSMENT_REQUIRED_SLOTS) {
      const accepted = acceptAnswer(next, ASSESSMENT_TEST_SLOT_ANSWERS[id], {
        slotId: id,
      });
  ...
  it("reducer path writes accepted slot answers; keyword result stays empty", () => {
    ...
    // Accepted slots are authoritative; legacy keyword result husk stays empty.
    assert.equal(state.slots.skill_to_improve.status, "filled");
    assert.equal(
      state.slots.skill_to_improve.answer,
      "I want to get better with small talks in everyday work meetings."
...
describe("assessment slot shadow observation — Step 2", () => {
  ...
  it("d. rejected shadow answers do not mutate authoritative result/covered", () => {
    ...
    // Confusion: acceptAnswer rejects and must not change slots or legacy husks.
  ...
  it("e. legacy keyword result/covered husks stay empty (Step 8)", () => {
    let { state } = reduceAssessmentLifecycle(createIdleAssessmentState(), {
      type: "START",
    });
    ({ state } = utter(state, "yes"));
...
describe("assessment Forge turn slot injection — Step 3", () => {
  ...
  it("Forge turn instructions receive lifecycle currentSlot as destination; null is ack-only", () => {
    for (const id of ASSESSMENT_SLOT_ORDER) {
      ...
      // Soft probe examples stay internal — turn prompts must not sticky-script wording.
      assert.doesNotMatch(turn, /Preferred next wording/i);
      assert.doesNotMatch(turn, /Coach-preferred discriminating wording/i);
      assert.match(turn, /DESTINATION/i);
      assert.doesNotMatch(turn, /Ask ONLY this/i);
      assert.match(turn, /Do NOT choose a different slot/i);
    }

    const nullTurn = buildAssessmentTurnInstructions(null);
    assert.match(nullTurn, /COMPATIBILITY DESTINATION: none provided/i);
    assert.match(nullTurn, /Brief acknowledgment and wait/i);
  ...
  it("prompt no longer authorizes the model to select the next slot", () => {
    ...
    assert.match(system, /Do NOT choose which slot id to ask next/i);
    ...
    assert.match(turn, /CLOSE-PATH TURN/i);
  });
...
describe("assessment snapshot (Step 6)", () => {
  it("builds snapshot from filled slots only — no invented answers", () => {
    let { state } = reduceAssessmentLifecycle(createIdleAssessmentState(), {
      type: "START",
    });
    ({ state } = utter(state, "yes"));
    ({ state } = utter(
      state,
      "I want to sound calmer when I present to leadership teams"
    ));
    ({ state } = utter(
      state,
      "It shows up most in weekly executive updates at work"
    ));

    const snap = buildAssessmentSnapshot(state, {
      practiceSessionId: "ps_test",
      sufficient: false,
      completedAt: "2026-08-11T12:00:00.000Z",
    });

    assert.equal(snap.version, 1);
    assert.equal(snap.consented, true);
    assert.equal(snap.sufficient, false);
    assert.equal(snap.practiceSessionId, "ps_test");
    assert.equal(snap.completedAt, "2026-08-11T12:00:00.000Z");
    assert.deepEqual(snap.filledSlotIds, [
      "skill_to_improve",
      "where_it_shows_up",
    ]);
    assert.equal(
      snap.answers.skill_to_improve,
      "I want to sound calmer when I present to leadership teams"
    );
    assert.equal(
      snap.answers.where_it_shows_up,
      "It shows up most in weekly executive updates at work"
    );
    assert.equal(snap.answers.what_goes_wrong, undefined);
    assert.equal(snap.answers.practice_time, undefined);
    // Keyword result must not be required for snapshot contents.
    for (const key of ASSESSMENT_CATEGORIES) {
      assert.equal(state.result[key], null);
    }
  });

  it("marks sufficient from slot completeness when meta omitted", () => {
    let { state } = reduceAssessmentLifecycle(createIdleAssessmentState(), {
      type: "START",
    });
    ({ state } = utter(state, "yes"));
    ...
    const snap = buildAssessmentSnapshot(state);
    assert.equal(isAssessmentSlotsComplete(state), true);
    assert.equal(snap.sufficient, true);
    assert.equal(snap.filledSlotIds.length, ASSESSMENT_REQUIRED_SLOTS.length);
    for (const id of ASSESSMENT_REQUIRED_SLOTS) {
      assert.ok(snap.answers[id]);
      assert.ok(ASSESSMENT_SLOT_LABELS[id]);
    }
  });

  it("does not invent answers for pending slots after consent only", () => {
    let { state } = reduceAssessmentLifecycle(createIdleAssessmentState(), {
      type: "START",
    });
    ({ state } = utter(state, "yes"));
