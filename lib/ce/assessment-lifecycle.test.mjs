...
describe("assessment slot primitives — Step 1 shadow state", () => {
  ...
  it("reducer still does not call acceptAnswer (shadow coexistence)", () => {
    let { state } = reduceAssessmentLifecycle(createIdleAssessmentState(), {
      type: "START",
    });
    ({ state } = utter(state, "yes"));
    ...
    // Legacy path may fill result; shadow slots stay pending until acceptAnswer.
    assert.equal(state.slots.skill_to_improve.status, "pending");
    assert.equal(state.slots.skill_to_improve.answer, null);
