...
describe("assessment Forge turn slot injection — Step 3", () => {
  it("resolveAssessmentTurnSlot prefers currentSlot then nextSlot", () => {
    const started = startAssessmentLifecycle();
    assert.equal(resolveAssessmentTurnSlot(started), null);

    const consented = { ...started, consented: true };
    assert.equal(resolveAssessmentTurnSlot(consented), "skill_to_improve");

    const asking = markSlotAsAsking(consented, "what_goes_wrong");
    assert.equal(resolveAssessmentTurnSlot(asking), "what_goes_wrong");
  });

  it("turn instructions include the app-selected currentSlot", () => {
  ...
  it("result/covered/completion remain applyCategories-owned after slot injection helpers", () => {
    let { state, effect } = reduceAssessmentLifecycle(
      createIdleAssessmentState(),
      { type: "START" }
    );
    ...
    // Shadow slots advance (Step 2); result still from applyCategories.
    assert.equal(state.slots.skill_to_improve.status, "filled");
    assert.ok(state.result.primaryGoal || state.result.realWorldContext);
    assert.equal(
      resolveAssessmentTurnSlot(state),
      state.currentSlot ?? nextSlot(state)
    );
    // Completion semantics unchanged — still structural, not slot-complete.
    if (state.assessmentStatus === "complete") {
      assert.equal(effect.type, "REQUEST_FINAL_RESPONSE");
      assert.equal(isAssessmentSlotsComplete(state), false);
    } else {
