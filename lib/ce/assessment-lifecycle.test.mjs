...
describe("assessment lifecycle — structural completion", () => {
  it("C. completion sets assessmentStatus = complete", () => {
    ...
    // Step 4: soft path via coveredCount no longer advances (applyCategories
    // no-ops covered). Complete via max substantive answers instead.
    let sawFinalRequest = false;
    for (let i = 0; i < ASSESSMENT_MAX_SUBSTANTIVE_ANSWERS; i++) {
      ({ state, effect } = utter(
        state,
        `I want clearer speaking in hard meetings and freeze under pressure number ${i + 1}`
      ));
