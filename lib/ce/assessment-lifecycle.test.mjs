...
/** Slot answers that pass Coach V2 sufficiency (not vague). */
...
function fillAllRequiredSlots(state) {
  ...
  for (const id of ASSESSMENT_REQUIRED_SLOTS) {
    ({ state, effect } = utter(state, USEFUL_SLOT_ANSWERS[id]));
...
describe("assessment slot primitives — Step 1 shadow state", () => {
  ...
  function fillAllRequired(state) {
    let next = state;
    for (const id of ASSESSMENT_REQUIRED_SLOTS) {
      const accepted = acceptAnswer(next, USEFUL_SLOT_ANSWERS[id], {
        slotId: id,
      });
  ...
  it("E2. vague answer is not_sufficient — currentSlot stays for clarify", () => {
    const base = markSlotAsAsking(consentedActiveState(), "skill_to_improve");
    const rejected = acceptAnswer(base, "I want to communicate better.");
    assert.equal(rejected.ok, false);
    ...
    const accepted = acceptAnswer(
      base,
      "I want to get my thoughts out clearly when I present at work."
    );
    assert.equal(accepted.ok, true);
    assert.equal(accepted.state.slots.skill_to_improve.status, "filled");
    assert.equal(accepted.state.currentSlot, "where_it_shows_up");
  });
...
describe("assessment Forge turn slot injection — Step 3", () => {
  ...
  it("Forge turn instructions receive lifecycle currentSlot as destination; null is ack-only", () => {
    ...
    assert.match(nullTurn, /Speak only a brief acknowledgment/i);
