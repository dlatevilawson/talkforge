...
describe("assessment prompt — closing + app ownership", () => {
  ...
  it("system prompt states app owns termination and performance diagnosis", () => {
    const system = buildAssessmentSystemInstructions();
    assert.match(system, /APPLICATION OWNS TERMINATION/i);
    assert.match(system, /What they want to improve when they speak/i);
    assert.match(system, /observable speaking\/conversation behaviors/i);
