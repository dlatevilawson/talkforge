...
import {
  extractAcceptedEvidenceFacts,
  scoreMechanismCandidates,
...
test("prompt: four gateways are dimensions + difficulty ladder", () => {
  ...
  assert.match(turn, /RAISE difficulty/i);
});
...
test("3. ambiguous retrieval vs idea-generation → uncertainty, not premature pick", () => {
  ...
  for (const s of scenarios) {
    assert.doesNotMatch(s.title, /interruption|saying no|executive presence/i);
    assert.doesNotMatch(s.mission, /interruption|saying no|executive presence/i);
  }
});
...
test("11. scenario recommendations cannot contain unsupported targets", () => {
  const diagnosis = synthesizeDiagnosis({
    ...
    what_goes_wrong: "The words disappear even though the idea is clear",
...
test("12. synthesized compatibility fields cannot become evidence", () => {
  ...
  assert.ok(userOnly.skill_to_improve);

  const facts = extractAcceptedEvidenceFacts(userOnly);
  assert.ok(!facts.some((f) => f.slotId === "behavior_to_change"));

  const snap = buildAssessmentSnapshot(state, { sufficient: true });
  assert.ok(snap.diagnosis);
  assert.equal(snap.answerSources?.behavior_to_change, "synthesized");
  // Re-synthesis from snapshot path must ignore synthesized
