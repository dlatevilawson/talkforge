...
describe("Forge Agent migration", () => {
  ...
  it("is registered on both deployment paths", () => {
    ...
    for (const pathName of ["greenfield", "existingProduction"]) {
      ...
      assert.ok(
        files.indexOf("20260912_forge_agent_reset_runs_purge.sql") >
          files.indexOf(migrationName),
        `${pathName} runs-purge corrective must follow forge agent tables`
      );
    }
  });

  it("resets runs through a definer helper without member policies", () => {
    const corrective = readFileSync(
      join(root, "supabase/migrations/20260912_forge_agent_reset_runs_purge.sql"),
      "utf8"
    );
    assert.match(corrective, /purge_forge_agent_runs_for_member/);
    assert.match(corrective, /security definer/);
    assert.match(corrective, /perform public\.purge_forge_agent_runs_for_member\(\)/);
