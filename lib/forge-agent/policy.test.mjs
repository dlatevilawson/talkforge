...
const actionsRoute = readFileSync(
  join(root, "app/api/forge-agent/actions/route.ts"),
  "utf8"
);
...
describe("Forge Agent Stage B inbox contract", () => {
  it("lists pending actions without materializing cues on member GET", () => {
    assert.match(actionsRoute, /requireForgeAgentRepo\(\)/);
    assert.match(
      actionsRoute,
      /gate\.repo\.listPendingActions\(gate\.userId\)/
    );
    assert.doesNotMatch(actionsRoute, /materializeDueActions/);
  });
});
