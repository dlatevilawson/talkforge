        ...
        update(values) {
          ...
          return {
            eq(column, value) {
              ...
              const chain = {
                abortSignal(signal) {
                  calls.push({ op: "abortSignal", signal });
                  return chain;
                },
                select(columns) {
...
describe("cron-guard", () => {
  ...
  it("passes abort signal to tick update and aborts hanging update", async () => {
    let aborted = false;
    const client = {
      calls: [],
      from(table) {
          ...
          update(values) {
            return {
              eq(column, value) {
                const chain = {
                  abortSignal(signal) {
                    client.calls.push({ op: "abortSignal", signal });
    ...
    const res = await updateCurrentTickRowWithTimeout(
      client,
      "tick-1",
      "completed",
      { durationMs: 5 },
    ...
    assert.equal(aborted, true);
    assert.ok(client.calls.some((c) => c.op === "abortSignal"));
  });

  it("verifies the production Supabase adapter calls .abortSignal(signal) on tick update", async () => {
    let passedSignal = null;
    let signalWasAbortedDuringCall = null;
    const client = {
      from(table) {
        assert.equal(table, "forge_agent_runs");
        return {
          update(values) {
                ...
                return {
                  select(columns) {
                    ...
                    return {
                      abortSignal(signal) {
                        passedSignal = signal;
                        signalWasAbortedDuringCall = signal.aborted;
                        return {
                          async single() {
                            return {
                              data: { id: "tick-prod", status: "completed" },
                              error: null,
                            };
                          },
                        };
    ...
    const result = await updateCurrentTickRowWithTimeout(
      client,
      "tick-prod",
      "completed",
      { durationMs: 10 }
    ...
    assert.equal(signalWasAbortedDuringCall, false);
  });
});
