...
describe("cron attempt identity and fail-closed", () => {
  ...
  const runner = readFileSync(join(root, "lib/forge-agent/cron-runner.ts"), "utf8");
  ...
  it("updates attempts by exact id and recovers without today's date", () => {
    assert.match(runner, /isUsableCronTick\(tick\)/);
    assert.match(cron, /p_cron_run_id: tickId/);
  ...
  it("bounds abandoned-tick reconciliation with abort signals and closes reserved attempts on budget-skip", () => {
    assert.match(cron, /reconcileAbandonedTickRows/);
    assert.match(cron, /select\("id, created_at"\)[\s\S]*?\.abortSignal\(signal\)/);
    assert.match(cron, /update\(\{[\s\S]*?stage: "abandoned"[\s\S]*?\.abortSignal\(signal\)/);
    assert.match(cron, /errorCode:\s*"CRON_TIME_BUDGET"/);
    assert.match(cron, /inputTokens:\s*0/);
    assert.match(cron, /outputTokens:\s*0/);
    assert.match(cron, /"FINALIZE"/);
    assert.match(cron, /"ATTEMPT"/);
    assert.match(cron, /select\("id"\)\s*\.maybeSingle\(\)/);
    assert.match(cron, /updateCurrentTickRowWithTimeout/);
  });
});
