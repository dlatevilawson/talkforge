...
const sql = readFileSync(
  join(root, "supabase/migrations/20260912185721_forge_agent_phase3_claim.sql"),
  "utf8"
);
...
describe("claim_due_forge_cues SQL contract", () => {
  it("is a public invoker RPC with service_role-only execute", () => {
    assert.match(sql, /create or replace function public\.claim_due_forge_cues\(p_limit integer\)/);
    ...
    assert.doesNotMatch(
      sql,
      /grant execute on function public\.claim_due_forge_cues\(integer\) to authenticated/
    );
    assert.doesNotMatch(
      sql,
      /grant execute on function public\.claim_due_forge_cues\(integer\) to anon/
    );
  });
