...
describe("assessment prompt discipline", () => {
  it("forbids mid-assessment restatement and coaching", () => {
    ...
    assert.match(system, /4–5 words|≤5 words/i);
  ...
  it("enforces one concrete question per turn in system + turn prompts", () => {
    ...
    assert.match(system, /ADAPTIVE DIAGNOSIS/i);
    assert.match(system, /Skip slots already answered|skip it/i);

    const turn = buildAssessmentTurnInstructions();
    assert.match(turn, /Exactly ONE concrete diagnostic question/i);
    assert.match(turn, /one question mark/i);
    assert.match(turn, /Pick the next uncovered diagnostic slot/i);
