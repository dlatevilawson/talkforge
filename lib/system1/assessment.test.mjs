...
describe("assessment prompt discipline", () => {
  ...
  it("hard-caps four anchors + two confirmations and bans practice transition", () => {
    const system = buildAssessmentSystemInstructions();
    for (const q of ASSESSMENT_ANCHOR_QUESTIONS) {
      assert.ok(system.includes(q), `missing anchor: ${q}`);
    }
    assert.match(system, /SIX questions/i);
    assert.match(system, /exactly TWO confirmation questions/i);
    assert.match(system, /7th content question/i);
    ...
    assert.doesNotMatch(
      system,
      /Adaptive follow-up on WHAT SPECIFICALLY makes it hard/
    );

    const turn = buildAssessmentTurnInstructions();
    assert.match(turn, /Six Forge questions max/i);
