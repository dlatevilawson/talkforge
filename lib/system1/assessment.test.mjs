...
describe("assessment prompt discipline", () => {
  it("forbids mid-assessment therapy mirroring and keeps adaptive coach contract", () => {
    ...
    assert.match(system, /CONVERSATIONAL JUDGMENT/i);
    assert.doesNotMatch(system, /nearly verbatim/i);

    const turn = buildAssessmentTurnInstructions("skill_to_improve");
    assert.match(turn, /acknowledge|clarif|challenge|Exactly ONE concrete question/i);
  ...
  it("enforces one concrete question per turn in system + turn prompts", () => {
    ...
    assert.match(system, /Never bundle two asks/i);
  ...
  it("keeps concrete diagnostic anchors and bans therapy-style framing", () => {
    ...
    assert.match(turn, /app owns completion/i);
    assert.match(turn, /Exactly ONE concrete question/i);
  ...
  it("example anchors are performance-diagnostic, not emotional", () => {
    ...
    assert.match(joined, /explain something out loud/);
    assert.match(joined, /practice each day/);
    assert.match(joined, /last time this happened/);
...
describe("one-question-per-turn classifiers", () => {
  ...
  it("allows a single concrete coach question", () => {
    const good =
      "Got it. When you're trying to explain something out loud, what usually happens?";
