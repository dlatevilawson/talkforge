...
describe("Coach Forge philosophy (CFP-001 + CFX-001)", () => {
  ...
  it("requires listen → analyze → respond before coaching", () => {
    ...
    assert.match(turn, /client-confirmed speaking turn/i);
    assert.match(turn, /You own the conversational move/i);
    assert.match(turn, /Do not monologue/i);
    assert.match(turn, /ambiguous contribution is not permission to advance/i);
    assert.match(turn, /Never reward a fragment with generic praise/i);
    assert.match(turn, /Never use 'quick check'/i);
    assert.match(turn, /do not volunteer analytical feedback/i);
    assert.match(turn, /stay fully in character/i);
    assert.match(turn, /explicitly asks for coaching|moved into debrief/i);
  });
  ...
  it("encodes minimal-intervention master communicator rules without token language", () => {
    ...
    assert.match(MINIMAL_INTERVENTION_COACHING_RULES, /DO NOT MAKE EVERY TURN A COACHING LESSON/);
    assert.match(MINIMAL_INTERVENTION_COACHING_RULES, /EVIDENCE-BACKED CRAFT/);
    assert.match(MINIMAL_INTERVENTION_COACHING_RULES, /spoken rep/);
    assert.doesNotMatch(MINIMAL_INTERVENTION_COACHING_RULES, /Burgoon|McCroskey|Petronio/);
  ...
  it("can ask what brought a first-time member in", () => {
    const text = buildOpeningSpeechInstructions({
      isReturning: false,
    });
  ...
  it("resumes an interrupted conversation instead of a first-session welcome", () => {
    const text = buildResumeOpeningSpeechInstructions({
      resumeBrief: "Member: I froze.\nForge: Say the first sentence.",
      eventTitle: "Difficult feedback",
    });
    assert.match(text, /resumed conversation/i);
    assert.match(text, /Difficult feedback/);
