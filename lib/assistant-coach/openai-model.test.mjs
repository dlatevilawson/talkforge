...
import {
  ...
  ASSISTANT_COACH_MAX_OUTPUT_TOKENS,
...
describe("Assistant Coach model fail-closed", () => {
  ...
  it("Real provider configured → openai mode selected", () => {
    ...
    assert.equal(ASSISTANT_COACH_MAX_OUTPUT_TOKENS, 400);
  });
  ...
  it("does not invoke the model factory after the anonymous turn cap", async () => {
    const repo = createMemoryAssistantCoachSessionRepository();
    const minted = await ensureAnonAssistantCoachSession({
      repository: repo,
      cookieSecret: TEST_SECRET,
    ...
    for (let i = 0; i < 10; i += 1) {
      await repo.appendMessage({
        ...
        turnIndex: i,
        role: "user",
        content: `Substantive diagnosis answer ${i}`,
        modelMeta: {},
      });
    }
    let factoryCalls = 0;
...
describe("openai-model scope", () => {
  it("does not silently return mockModel when key absent on hosted env", () => {
    ...
    assert.match(src, /reasoning:\s*\{\s*effort:\s*"minimal"/);
    assert.match(src, /text:\s*\{\s*verbosity:\s*"low"/);
    assert.match(src, /max_output_tokens:\s*ASSISTANT_COACH_MAX_OUTPUT_TOKENS/);
  });
});
