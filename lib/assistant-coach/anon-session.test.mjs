...
describe("4B.3 ensureAnonAssistantCoachSession", () => {
  ...
  it("restores and heals a legacy gated cookie row for historical recovery only", async () => {
    ...
    const sealed = sealAnonCookieValue(rawSecret, TEST_SECRET);
    ...
    assert.equal(restored.session.id, legacy.id);
  ...
  it("unique-insert race adopts the winning session only for UniqueConflictError", async () => {
    const { AssistantCoachUniqueConflictError } = await import(
      "./session-repository.ts"
    );
    const repo = createMemoryAssistantCoachSessionRepository();
    const mintKey = generateAnonSecret();
    const hash = hashAnonSecret(mintKey);
    const winner = await repo.createSession({ anonKeyHash: hash });
    let lookups = 0;
    const wrapping = {
      ...
      async getSessionByAnonKeyHash(h) {
        lookups += 1;
        // Miss the pre-insert fast path once, then return the winner on adopt.
        if (lookups === 1) return null;
        return repo.getSessionByAnonKeyHash(h);
      },
      async getSession(id) {
    ...
    const adopted = await ensureAnonAssistantCoachSession({
      repository: wrapping,
      cookieSecret: TEST_SECRET,
      mintKey,
      secureCookie: false,
    });
    assert.equal(adopted.outcome, "restored");
    assert.equal(adopted.session.id, winner.id);
    assert.equal(adopted.rawSecret, mintKey);
    assert.notEqual(adopted.session.anonKeyHash, mintKey);
  });

  it("adopts and heals a legacy gated unique-conflict winner", async () => {
    const { AssistantCoachUniqueConflictError } = await import(
      "./session-repository.ts"
    );
    const now = new Date("2026-09-07T08:00:00.000Z");
    ...
    const mintKey = generateAnonSecret();
    const hash = hashAnonSecret(mintKey);
    const winner = await repo.createSession({ anonKeyHash: hash, now });
    let lookups = 0;
    const wrapping = {
      async createSession(input) {
        throw new AssistantCoachUniqueConflictError(input.anonKeyHash);
      },
      async getSessionByAnonKeyHash(value) {
        lookups += 1;
        if (lookups === 1) return null;
        return repo.getSessionByAnonKeyHash(value);
      },
      async normalizeLegacyGatedSession(id, at) {
        return repo.normalizeLegacyGatedSession(id, at);
      },
      async getSession(id) {
        return repo.getSession(id);
      },
      async markExpiredIfPast(id, at) {
        return repo.markExpiredIfPast(id, at);
      },
      async getDraft(id) {
        return repo.getDraft(id);
      },
    ...
    assert.equal(adopted.outcome, "restored");
    assert.equal(adopted.session.id, winner.id);
    assert.equal(adopted.session.status, "active");
    assert.equal((await repo.getSession(winner.id)).status, "active");
  });

  it("session-ok + draft-fail does not report successful mint", async () => {
    const repo = createMemoryAssistantCoachSessionRepository();
    const mintKey = generateAnonSecret();
    const wrapping = {
      async createSession(input) {
        // Persist session (and draft in memory), then simulate the production
        // failure mode: draft upsert throws after session insert.
        await repo.createSession(input);
        throw new Error(
          "assistant_coach_profile_drafts insert failed: simulated"
        );
      },
      async getSessionByAnonKeyHash(h) {
        return repo.getSessionByAnonKeyHash(h);
    ...
    await assert.rejects(
      ...
      /profile_drafts insert failed/
    );
    // Session row may exist from the partial write; mint must still have failed
    // (not converted into a successful adopt).
    const orphan = await repo.getSessionByAnonKeyHash(hashAnonSecret(mintKey));
    assert.ok(orphan);
  });

  it("incomplete session without draft is not adopted as a successful mint", async () => {
    const { AssistantCoachUniqueConflictError } = await import(
      "./session-repository.ts"
    );
    const repo = createMemoryAssistantCoachSessionRepository();
    const mintKey = generateAnonSecret();
    const hash = hashAnonSecret(mintKey);
    const incomplete = await repo.createSession({ anonKeyHash: hash });
    // Simulate draft never landing (or rolled back) while session remains active.
    ...
    await assert.rejects(
      ...
      /without a complete profile draft/
    );
    assert.equal(incomplete.id, (await repo.getSession(incomplete.id)).id);
  });
...
describe("4B.3 route handler", () => {
  ...
  it("draft creation failure returns 500 and does not Set-Cookie", async () => {
    const base = createMemoryAssistantCoachSessionRepository();
    const failingRepo = {
      async createSession(input) {
      ...
      async getSessionByAnonKeyHash(h) {
        return base.getSessionByAnonKeyHash(h);
      },
      async getSession(id) {
        return base.getSession(id);
      },
      async markExpiredIfPast(id, now) {
        return base.markExpiredIfPast(id, now);
      },
      async getDraft(id) {
        return base.getDraft(id);
      },
      async saveDraft(draft) {
        return base.saveDraft(draft);
      },
    };
    const mintKey = generateAnonSecret();
    const res = await handleAssistantCoachSessionRequest(
      ...
      depsWith(failingRepo)
    );
    assert.equal(res.status, 500);
    assert.equal(getSetCookie(res).length, 0);
    const body = await res.json();
    assert.match(body.error || "", /Unable to establish/i);
    assert.equal(body.session, undefined);
    assert.equal(body.rawSecret, undefined);
  });

  it("concurrent POST with same Idempotency-Key converges on one session", async () => {
    const repo = createMemoryAssistantCoachSessionRepository();
    const mintKey = generateAnonSecret();
    const [r1, r2] = await Promise.all([
      handleAssistantCoachSessionRequest(
        new Request("http://localhost/api/assistant-coach/session", {
          method: "POST",
          headers: { "Idempotency-Key": mintKey },
        }),
    ...
    assert.ok(getSetCookie(r1).length >= 1);
    assert.ok(getSetCookie(r2).length >= 1);
  });
...
describe("4B.3 scope guards", () => {
  it("does not introduce guest revival or a model runtime in this package surface", async () => {
