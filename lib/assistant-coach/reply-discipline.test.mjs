...
describe("Starter-turn response discipline", () => {
  ...
  it("does not rewrite direct questions or non-starter turns", () => {
    ...
    assert.equal(
      ...
      "Who is usually in the room when it happens?"
    );
  });
  ...
  it("blocks the exact coaching failures from the founder walkthroughs", () => {
    const examples = [
      {
        reply:
          "Frank, I’m excited about partnering—can we start by aligning on what success in the next 6–12 months looks like and where I’d own outcomes, so you can see exactly how I add value?",
        intervention: null,
      },
      {
        reply:
          "I need our interactions to stay professional—harassing comments or behavior must stop immediately; if this happens again, I will report it to HR.",
        intervention: {
          kind: "wording",
          summary:
            "Tell the boss directly that the behavior must stop and mention escalation to HR.",
          groundedInCategories: ["communication_context"],
        },
      },
    ];
    ...
    for (const example of examples) {
      const out = disciplineAssistantCoachOutput({
        ...example,
        userMessages: ["Harassment", "Directly"],
      });
      assert.equal(out.reply, UNDERSTANDING_FALLBACK);
      assert.equal(out.intervention, null);
      assert.equal(out.withheldIntervention, example.intervention !== null);
      assert.doesNotMatch(out.reply, /\b(?:I|my|we|our)\b/);
    }
  });

  it("rejects dialogue-shaped and coaching questions", () => {
    for (const reply of [
      "Could you say, “Can I explain why I should be your partner?”",
      "Would you practice the opener before speaking to Frank?",
    ]) {
      const out = disciplineAssistantCoachOutput({
        reply,
        intervention: null,
        userMessages: ["Directly"],
      });
      assert.equal(out.reply, UNDERSTANDING_FALLBACK);
    }
  });
...
describe("Runtime withholds a script dump from conversion", () => {
  it("does not gate after a curriculum reply", async () => {
    ...
    assert.equal(second.reply, UNDERSTANDING_FALLBACK);
