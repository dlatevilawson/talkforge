...
describe("claimAssistantCoachSession", () => {
  it("claims gated anon session onto member LP and is idempotent", async () => {
    ...
    assert.equal(second.draftProfile.evidenceLedger.length, 2);
  });

  it("rejects claim by a different user without disclosing ownership", async () => {
    const repo = createMemoryAssistantCoachSessionRepository();
    const minted = await ensureAnonAssistantCoachSession({
      repository: repo,
      cookieSecret: TEST_SECRET,
    ...
    await assert.rejects(
      ...
      (err) =>
        err instanceof AssistantCoachClaimError &&
        err.status === 404 &&
        !/owner|belongs|account/i.test(err.message)
    );
  });
});
...
describe("HTTP claim", () => {
  it("401 without auth; 200 with cookie + auth", async () => {
    ...
    profiles.profile.goals = ["Board presentations"];
    ...
    assert.doesNotMatch(body.confirmation.workingOn, /board/i);
    assert.doesNotMatch(body.confirmation.identifiedMoment, /board/i);
    void hashAnonSecret;
  });
});
