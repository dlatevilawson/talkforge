...
describe("Assistant Coach turn prompt", () => {
  it("understands the visitor; does not train like Forge", () => {
    ...
    assert.match(prompt, /next useful diagnostic question/i);
    assert.match(prompt, /Every reply must be exactly one focused question/i);
    assert.match(prompt, /Who’s the interview with/i);
    assert.match(prompt, /Bad: "You’re getting ready for an interview/i);
    assert.match(prompt, /Harassment/);
    assert.match(prompt, /Advice, recommendations, strategies/i);
    assert.match(prompt, /Writing in first person as though you are the member/i);
    ...
    assert.doesNotMatch(prompt, /ONE short usable first move/i);
    assert.doesNotMatch(prompt, /"intervention"/i);
