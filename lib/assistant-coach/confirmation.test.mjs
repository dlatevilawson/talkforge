...
describe("Confirmation view", () => {
  it("maps interview evidence into the four human fields", () => {
    ...
    assert.equal(view.firstWork, view.identifiedMoment);
    assert.doesNotMatch(view.firstWork, /Stay clear and structured/i);
    assert.doesNotMatch(view.firstWork, /We’ll keep learning|We'll keep learning/i);
    assert.equal(view.canContinue, true);
  });

  it("does not treat a life topic as the identified moment", () => {
    const profile = emptyLivingProfile("u1", "Avery");
    profile.goals = ["Considering a career change"];
    profile.evidenceLedger = [
      ev("g", "Considering a career change", "communication_goal"),
    ];
    const view = buildConfirmationView(profile);
    assert.equal(view.identifiedMoment, "");
    assert.equal(view.canContinue, false);
    const recovered = buildConfirmationView(profile, {
      userMessages: [
        "I’m considering a career change.",
        "I need to tell my wife I’m considering a career change",
      ],
    });
    assert.match(recovered.identifiedMoment, /tell my wife/i);
...
describe("HTTP confirm", () => {
  it("returns practiceHref and keeps purpose untouched", async () => {
    ...
    assert.equal(body.identifiedMoment, "Tell me about yourself");
