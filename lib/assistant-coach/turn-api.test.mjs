...
describe("4B.4 runAssistantCoachTurn", () => {
  ...
  it("withholds coaching dialogue and cannot trigger the value gate", async () => {
    const repo = createMemoryAssistantCoachSessionRepository();
    const minted = await ensureAnonAssistantCoachSession({
      repository: repo,
      cookieSecret: TEST_SECRET,
    ...
    const result = await runAssistantCoachTurn({
      ...
      session: minted.session,
      message: "Directly",
      clientTurnId: "diagnosis-only-1",
      model: async () => ({
        reply:
          "I need our interactions to stay professional—harassing comments must stop; if this happens again, I will report it to HR.",
        observations: [
          {
            text: "Wants to address workplace harassment directly",
        ...
        intervention: {
          ...
          groundedInCategories: ["desired_outcome"],
        },
      }),
    });
    ...
    const messages = await repo.listMessages(minted.session.id);
    assert.equal(messages.at(-1)?.modelMeta.interventionAccepted, false);
    assert.equal(
      messages.at(-1)?.modelMeta.interventionRejectReason,
      "diagnosis_only"
    );
  });
