/**
 * First-user vertical slice wiring, including the topic-card Coach entry.
...
describe("Vertical slice contract", () => {
  ...
  it("Coach topic selection goes directly to the public Forge route", () => {
    const client = read("app/coach/AssistantCoachClient.tsx");
    assert.match(client, /router\.push\(`\/forge\?topic=/);
    assert.doesNotMatch(client, /signup|login|\/coach\/confirm/);
  });

  it("governance retires the pre-Forge profile confirmation path", () => {
    const journey = read("atos/product/AC-JOURNEY-001-first-user-architecture.md");
    assert.match(journey, /no profile form or confirmation card/i);
    assert.match(journey, /no pre-Forge signup, signin/i);
    assert.match(journey, /seven-topic card grid/i);
  });
