...
describe("Vertical slice contract", () => {
  ...
  it("activates without a second confirmation and enters Forge", () => {
    ...
    const client = read("app/coach/AssistantCoachClient.tsx");
    const activation = read("app/coach/activate/route.ts");
    ...
    assert.match(activation, /activateAssistantCoachProfile/);
    assert.doesNotMatch(activation, /\/coach\/confirm/);
  });

  it("proxy protects activation and practice while retaining legacy confirm", () => {
  ...
  it("Looks right carries identifiedMoment into Forge and never falls back to bare practice", () => {
    ...
    assert.match(realtime, /practiceContext/);
    ...
    assert.match(arena, /authoritativePracticeContext/);
