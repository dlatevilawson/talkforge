...
describe("4B.5 semantic value", () => {
  ...
  it("diagnosis never becomes semantic coaching value", () => {
    assert.equal(
      computeHasExperiencedValue({
        evidenceLedger: [goalEvidence(), frictionEvidence()],
        profileInsights: [],
...
describe("Founder path: diagnosis never triggers a semantic gate", () => {
  it("withholds an actionable intervention and keeps diagnosis open", async () => {
    ...
    assert.equal(third.reply, "What feels hardest about handling this conversation?");
    ...
    assert.equal(fourth.gate.mustAuthenticateToContinue, false);
    const after = await repo.getSession(minted.session.id);
    assert.equal(after.hasExperiencedValue, false);
    assert.equal(after.status, "active");
    const draft3 = await repo.getDraft(minted.session.id);
    assert.ok((draft3.profileJson.evidenceLedger?.length ?? 0) >= 2);
  });

  it("ignores model interventions even after discovery is ready", async () => {
    const repo = createMemoryAssistantCoachSessionRepository();
    const minted = await ensureAnonAssistantCoachSession({
      repository: repo,
      cookieSecret: TEST_SECRET,
    ...
    assert.equal(second.session.hasExperiencedValue, false);
    assert.equal(second.gate.mustAuthenticateToContinue, false);
    assert.equal(second.reply, "What feels hardest about handling this conversation?");
    ...
    const third = await runAssistantCoachTurn({
      ...
      clientTurnId: "v3",
      model: async () => ({
        reply: "When does your throat begin to tighten?",
        observations: [],
      }),
    });
    assert.equal(third.gate.mustAuthenticateToContinue, false);
...
describe("Anonymous diagnosis turn cap", () => {
  it("reopens a legacy value-gated session below the turn cap", async () => {
    const repo = createMemoryAssistantCoachSessionRepository();
    ...
    const result = await runAssistantCoachTurn({
      ...
      clientTurnId: "reopened-1",
      model: async () => {
        calls += 1;
        return {
          reply: "What still feels unresolved about this conversation?",
          observations: [],
        };
      },
    });
    ...
    assert.equal(result.gate.hasExperiencedValue, false);
  ...
  it("auth member with claimed session is not hard-gated by anon value rule", async () => {
    ...
    const result = await runAssistantCoachTurn({
      ...
      model: async () => ({
        reply: "What part of the opening still feels uncertain?",
        observations: [],
      }),
    });
    assert.equal(result.idempotentReplay, false);
    assert.match(result.reply, /uncertain/i);
  });
});
