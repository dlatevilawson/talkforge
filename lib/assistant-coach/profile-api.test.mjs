...
describe("Coach wizard profile API", () => {
  it("verifies and persists only memberPracticeProfile in the existing draft", async () => {
    ...
    const retry = await handleAssistantCoachProfileRequest(request(), deps);
    assert.equal(retry.status, 200);
    assert.equal((await repository.getDraft(session.id)).version, 2);
  });
  ...
  it("activates authenticated members immediately with an explicit destination", async () => {
    const { repository, session, deps } = await setup();
    let activation = null;
    const response = await handleAssistantCoachProfileRequest(request(), {
      ...deps,
      resolveAuthUserId: async () => "member_direct",
      activateAuthenticated: async (input) => {
        activation = input;
        return { destination: COACH_WIZARD_PRACTICE_DESTINATION };
      },
    });
    assert.equal(response.status, 200);
    const body = await response.json();
    assert.equal(body.destination, COACH_WIZARD_PRACTICE_DESTINATION);
    assert.equal(activation.userId, "member_direct");
    assert.equal(
      activation.anonKeyHash,
      hashAnonSecret(RAW_SECRET)
    );
