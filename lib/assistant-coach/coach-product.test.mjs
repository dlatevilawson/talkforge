...
describe("Coach product copy", () => {
  ...
  it("renders the governed card wizard instead of conversational starters", () => {
    const client = read("app/coach/AssistantCoachClient.tsx");
    assert.match(client, /Pick your moments/);
    assert.match(client, /PRACTICE_TOPIC_CATALOG\.map/);
    assert.match(client, /Narrow the context/);
    assert.match(client, /Your Coach profile/);
    assert.match(client, /type="button"/);
    assert.match(client, /aria-pressed=\{selected\}/);
    assert.match(client, /role="radiogroup"/);
    assert.doesNotMatch(client, /COACH_STARTERS\.map|sendMessage\(/);
  });
});
...
describe("Coach product scope guards", () => {
  ...
  it("keeps the wizard accessible across narrow and wide viewports", () => {
    const client = read("app/coach/AssistantCoachClient.tsx");
