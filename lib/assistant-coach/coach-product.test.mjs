...
describe("Coach product copy", () => {
  it("opens on the communication moment, not generic chat", () => {
    assert.equal(
      COACH_OPENING,
    ...
    assert.doesNotMatch(COACH_OPENING, /on your mind|how can I help/i);
  });

  it("offers the five Founder-directed conversation starters in order", () => {
    assert.deepEqual(
      COACH_STARTERS.map((starter) => starter.label),
      [
        "Interview",
        "Salary negotiation",
        "Difficult feedback",
    ...
    assert.equal(new Set(COACH_STARTERS.map((starter) => starter.id)).size, 5);
  });

  it("starts named conversations and leaves Something else for custom input", () => {
    for (const starter of COACH_STARTERS.slice(0, -1)) {
      assert.match(starter.message ?? "", /prepar/i);
      assert.ok((starter.message ?? "").length > starter.label.length);
    }
    assert.equal(COACH_STARTERS.at(-1)?.message, null);
  });

  it("provides concise contextual guidance for every starter", () => {
    assert.deepEqual(
      Object.fromEntries(
        COACH_STARTERS.map((starter) => [starter.id, starter.placeholder])
      ),
      {
        interview: "Describe the role or company…",
        "salary-negotiation": "Who are you negotiating with…",
        "difficult-feedback": "Who needs the feedback…",
        "setting-a-boundary": "Who is the boundary with…",
        "something-else": "Describe the conversation…",
      }
    );
    assert.equal(getCoachComposerPlaceholder(null), "Name your conversation…");
    for (const starter of COACH_STARTERS) {
      assert.equal(
        getCoachComposerPlaceholder(starter.id),
        starter.placeholder
      );
    }
  });

  it("infers restored starter context only from an exact first message", () => {
    for (const starter of COACH_STARTERS.slice(0, -1)) {
      assert.equal(inferCoachStarterId(starter.message), starter.id);
    }
    assert.equal(inferCoachStarterId(null), null);
    assert.equal(inferCoachStarterId("I’m preparing for an interview"), null);
    assert.equal(inferCoachStarterId("A different conversation."), null);
  });
  ...
  it("renders starters only in the empty thread with accessible button behavior", () => {
    const client = read("app/coach/AssistantCoachClient.tsx");
    assert.match(client, /messages\.length === 0/);
    assert.match(client, /COACH_STARTERS\.map/);
    assert.match(client, /role="group"/);
    assert.match(client, /aria-label="Common conversation types"/);
    assert.match(client, /type="button"/);
    assert.match(client, /sendMessage\(starter\.message\)/);
    assert.match(client, /composerInputRef\.current\?\.focus\(\)/);
    assert.match(client, /setSelectedStarterId\(starter\.id\)/);
    assert.match(client, /getCoachComposerPlaceholder\(selectedStarterId\)/);
    assert.match(client, /inferCoachStarterId/);
    assert.match(client, /message\.role === "user"/);
  });
});

describe("Diagnosis-only value boundary", () => {
  it("diagnosis and model interventions never become coaching value", () => {
    ...
    assert.equal(valued, false);
    // Diagnosis never invents a completeness or coaching-value field.
...
describe("Coach transcribe HTTP", () => {
  ...
  it("turn-capped session cannot transcribe (no STT spend)", async () => {
    const { repo, minted } = await mint();
...
describe("Coach product scope guards", () => {
  ...
  it("keeps the composer docked: only the thread scrolls", () => {
    ...
    assert.match(css, /--ac-vvt/);
    assert.match(css, /position:\s*fixed/);
    assert.match(css, /overflow-x:\s*hidden/);
    assert.match(css, /width:\s*auto/);
    assert.match(css, /overflow-wrap:\s*break-word/);
    assert.match(css, /ac-composer-dock/);
    assert.match(css, /ac-composer-dock textarea[\s\S]*min-width:\s*0/);
    assert.match(css, /@media \(max-width:\s*420px\)/);
    assert.match(
      css,
      /ac-composer-dock \.ac-btn-icon[\s\S]*padding-inline:\s*0\.5rem/
    );
    const layout = read("app/layout.tsx");
    assert.match(layout, /interactiveWidget:\s*"resizes-content"/);
    assert.doesNotMatch(client, /window\.scrollTo/);
    assert.match(client, /offsetTop/);
    assert.match(client, /ac-composer-dock/);
    assert.match(client, /enterKeyHint/);
  });
