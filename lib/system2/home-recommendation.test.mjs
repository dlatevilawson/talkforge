...
describe("Adaptive home recommendation contract", () => {
  ...
  it("does not treat billing unavailable as a Founding Pass lock", () => {
    assert.deepEqual(
      homeEntitlementFromPractice({
    ...
    assert.deepEqual(
      homeEntitlementFromPractice({ canStartPractice: true, reason: "pro" }),
      { status: "open" }
    );
  });
...
describe("Continuity Home implements the Coach contract", () => {
  ...
  it("does not render active training controls when practice is limited", () => {
    ...
    assert.match(home, /homeEntitlementFromPractice/);
