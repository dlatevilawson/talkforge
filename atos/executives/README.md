# Executive Systems Package

| Field | Value |
|---|---|
| **Document ID** | EXEC-ROOT |
| **Version** | 1.0.0-m6 |
| **Status** | Draft |
| **Owner** | Founder |
| **AI Steward** | Atlas |
| **Human Approver** | Founder |
| **Review Cycle** | Each milestone |
| **Dependencies** | SPEC-006, STD-003, RES-001, REG-EXEC |
| **Related Documents** | charters/*, collaboration.md, MAN-001…MAN-003 |
| **Approval History** | 2026-07-18 — M6 created |
| **Change Log** | 2026-07-18 — Executive Systems package root |

---

| Path | Purpose |
|---|---|
| [charters/](charters/) | Executive charters (Atlas, Sentinel, Founder) |
| [collaboration.md](collaboration.md) | Collaboration interfaces |
| Manuals | `atos/manuals/MAN-00x-*.md` |

## Atlas Program

Founder-ratified Phase 0 Design Contract: [`atlas-program/PHASE-0-DESIGN-CONTRACT.md`](atlas-program/PHASE-0-DESIGN-CONTRACT.md) (RES-003).
