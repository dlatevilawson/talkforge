# EXEC-001 — TalkForge Execution Plan

| Field | Value |
|---|---|
| **Document ID** | EXEC-001 |
| **Title** | TalkForge Execution Plan |
| **Version** | 1.0.0 |
| **Status** | **Active — Step 1 awaiting Founder review** |
| **Owner** | Founder |
| **AI Steward** | Atlas |
| **Created** | 2026-08-01 |
| **Authority** | Founder directive — company operating system |
| **Related** | [Idea Vault](../knowledge/working/idea-vault/README.md) · KA-001 · TEA-001 · ROADMAP-001 · Constitution · Decision Ledger |

---

## Purpose

Create a permanent operating system for TalkForge that captures every important philosophy, decision, feature, research finding, and implementation plan so nothing valuable is lost and every future decision remains aligned with the company’s mission.

---

## Hard gates (binding)

1. **No new features** are built until they have been documented, categorized, reviewed for blind spots, and assigned a place in the roadmap.
2. **Every step requires Founder review and approval** before the next step may begin.
3. Agents **must not** start Step *N+1* while Step *N* is `Awaiting Founder review` or `Blocked`.

---

## Phases

| Phase | Goal | Status |
|---|---|---|
| **1 — Foundation** | Build the company’s knowledge base | **In progress** (Step 1 only) |
| **2 — Strategy** | Convert knowledge into executable strategy | ⬜ Not Started (blocked until Phase 1 complete) |
| **3 — Product Execution** | Build TalkForge one validated system at a time | ⬜ Not Started (blocked until Phase 2 complete) |

---

## Phase 1 — Foundation (Knowledge Base)

| Step | Deliverable | Status | Path |
|---|---|---|---|
| **1** | Living Idea Vault | **Awaiting Founder review** | [`atos/knowledge/working/idea-vault/`](../knowledge/working/idea-vault/) |
| **2** | TalkForge Constitution v1.0 | ⬜ Blocked | *(not started)* |
| **3** | Decision History (Ledger) | ⬜ Blocked | Existing: `atlas/decisions.md` — formal ledger step later |
| **4** | Research Library | ⬜ Blocked | Partial: `atos/product/research/`, HBF — formal library later |
| **5** | Blind Spot Register | ⬜ Blocked | *(not started)* |
| **6** | Future Parking Lot / Backlog | ⬜ Blocked | *(not started)* |
| **7** | Founder Journal | ⬜ Blocked | *(not started)* |

---

## Phase 2 — Strategy

| Step | Deliverable | Status |
|---|---|---|
| **8** | Fully categorized knowledge base | ⬜ Blocked |
| **9** | Verified Knowledge Base (fact-check marks) | ⬜ Blocked |
| **10** | Dependency Map | ⬜ Blocked |
| **11** | Company Priority Matrix | ⬜ Blocked |
| **12** | Master Roadmap | ⬜ Blocked |

---

## Phase 3 — Product Execution

| Step | Focus | Status |
|---|---|---|
| **13** | System 1 — Living Profile, Memory, Evidence, Understanding, Reflection, Provenance, Conversation Lifecycle | ⬜ Blocked |
| **14** | System 2 — Readiness, Recommendation, Pedagogy, Personalization, Adaptive Homepage | ⬜ Blocked |
| **15** | Conversation Engine — Simulations, Personas, Voice, Interruptions, Scoring | ⬜ Blocked |
| **16** | User Experience — Homepage, Missions, Progress, Canvas, Moments, Timeline | ⬜ Blocked |
| **17** | User Testing — Trust, Clarity, Engagement, Confidence, Learning, Readiness, Retention | ⬜ Blocked |
| **18** | Iterate — improve existing systems before introducing new ones | ⬜ Blocked |

---

## Operating Rules

| # | Rule |
|---|---|
| 1 | Capture every meaningful idea before discussing the next one. |
| 2 | Every major idea must be categorized. |
| 3 | Every important claim must be fact-checked. |
| 4 | Every feature must pass the Blind Spot Register. |
| 5 | Every approved idea must be linked to the roadmap. |
| 6 | Every major decision must be recorded in the Decision Ledger. |
| 7 | Build only after the foundation and strategy are complete. |

---

## Immediate next step (agents)

**Stop after Step 1.** Present the Idea Vault for Founder review.

Do **not** begin Step 2 (Constitution) or any later step until the Founder explicitly approves Step 1.

---

## Approval log

| Date | Step | Decision |
|---|---|---|
| 2026-08-01 | Plan adopted as EXEC-001 | Decision 048 — execution gated per Founder |
| 2026-08-01 | Step 1 Idea Vault built | **Awaiting Founder review** |
| — | Step 1 | ☐ Approve · ☐ Revise · ☐ Reject |

---

## Change log

| Version | Date | Change |
|---|---|---|
| 1.0.0 | 2026-08-01 | Initial capture of Founder Execution Plan; Step 1 Idea Vault delivered for review |
