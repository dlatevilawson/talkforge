...
| [AUDIT-001.2](atos/product/AUDIT-001.2-conditional-go-reaudit.md) | Conditional GO re-audit — **GO** prod migration/hardening · **NO-GO** features (**Decision 059 carve-out redirected by Decision 060**) |
| [REMEDIATE-002](atos/product/REMEDIATE-002-conditional-go.md) | Conditional GO implementation report |
| [EXEC-VERIFY-001](atos/product/EXEC-VERIFY-001-final-architecture-certification.md) | Final architecture certification — **NO-GO** general features · **Decision 060** controls AC journey carve-out |
| [AC-JOURNEY-001](atos/product/AC-JOURNEY-001-first-user-architecture.md) | Deterministic first-user Coach card wizard (Decision 060) |
| [PHASE4B-AC](atos/product/PHASE4B-AC-IMPLEMENTATION-SEQUENCE.md) | Deterministic wizard/auth activation implementation sequence |
...
- **Certification (EXEC-VERIFY-001):** Baseline remains **NO-GO** for general feature development. **Carve-out:** Decision 059 historically authorizes the Assistant Coach first-user track; Decision 060 redirects its active architecture. Proceed only via slices in [PHASE4B-AC-IMPLEMENTATION-SEQUENCE](atos/product/PHASE4B-AC-IMPLEMENTATION-SEQUENCE.md). Do not lift FREEZE-001 for held identity PRs or treat this as a general feature GO.
- **Coach first-user track (Decision 060):** Phase 1 **Pick your moments** (exact topic cards, ordered multi-select 1–3; only Something else may reveal bounded text) → Phase 2 **Narrow the context** (audiences multi-select, pattern single-select, urgency single-select; CTA Diagnose) → Phase 3 deterministic Living Profile verification (focus areas, selected-pattern template, initial Forge target from first topic + first audience). **Adjust** returns to Phase 2 prefilled. **Looks right** gates guests on auth and activates members. Persist declarations in the one Living Profile `member_practice_profile` JSONB field with member provenance/timestamps; it is not a parallel profile or System 1 inference. Auth is required before Forge; no desired-outcome question, discovery LLM, semantic gate, anonymous Coach turns, guest revival, or duplicate post-auth confirmation. Signed server anon session/14-day TTL remain for provisional drafts. Assessment remains reachable but demoted.
...
- Do **not** ship new product features that are not documented, categorized, blind-spot reviewed, and roadmap-linked — except continuation of already-ratified tracks (e.g. CE-001 under RES-013; deterministic Coach first-user Phase 4B under Decisions 059–060 / IV-PROD-009 / IV-UX-011 / BS-018).
