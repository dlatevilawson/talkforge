...
- **Readiness measurement (Decision 063):** only the READINESS-MEASUREMENT-001 foundation is authorized: deterministic contract code, versioned assessment/signal/evidence storage, owner-only reads, service-only writes, and tests. No production migration apply, model evaluator, member UI, training program, pricing change, or voice-pipeline change without a later Founder gate. Readiness is never a 0–100 score or cross-member comparison; null is never zero; only sufficient current-session evidence may affect a band or trend.
