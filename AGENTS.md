...
- **Assistant Coach first-user track (Decision 059 + Founder correction 2026-09-07):** Shipping conversational Assistant Coach is diagnosis-only: one focused question, validated observations, no advice, wording, rehearsal, strategy, or intervention. Model interventions are discarded server-side; `hasExperiencedValue` does not gate diagnosis. The configurable anonymous turn cap is the sole safety/economic stop while the deterministic card-wizard replacement is implemented. Signed HttpOnly cookie + server anon session (no guest revival), 14-day anon TTL, AC primary landing CTA, Assessment demoted but kept, soft email verify, and System 1 evidence ownership remain binding.
